...
for i in range(num_ic):
    ...
    # ax.scatter3D(outputs[i][:, 0], outputs[i][:, 1], outputs[i][:, 2],s = 50, alpha = 0.3, cgitolor=selected_color, zorder=i, edgecolors='none')
    ...
    plt.plot(outputs[i][:, 2], outputs[i][:, 1], alpha = 0.75, color=selected_color, linewidth=1.5, zorder=i)
    # plt.figure()
    # plt.plot(outputs[i][:, 0], outputs[i][:, 2], alpha = 0.75, color=selected_color, linewidth=1, zorder=i)
